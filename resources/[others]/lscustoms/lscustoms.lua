...
RegisterNetEvent('lsCustomsCmd')
AddEventHandler('lsCustomsCmd', function()
	lscustomsCmd()
end)

function lscustomsCmd()
	local ped = LocalPed()
	if IsPedSittingInAnyVehicle(ped) then
		...
		if DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)))  then
			...
			TriggerEvent('chat:addMessage', {template = '<div class="chat-message server"><b>ERROR:</b> You are not the driver of the vehicle!</div>'})
        	return false 
		end
	else
		TriggerEvent('chat:addMessage', {template = '<div class="chat-message server"><b>ERROR:</b> You are not in a vehicle</div>'})
        return false 
	end
end
...
RegisterNetEvent('LSC:GetMyVehInfo')
AddEventHandler('LSC:GetMyVehInfo', function(veh, modifs, color, color2, neon, smoke, wTint, wType, bpTyre)
	if veh ~= nil then
		myveh.vehicle = veh
		myveh.model = GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()
		myveh.color = color
		myveh.extracolor = color2
		myveh.neoncolor = neon
		myveh.smokecolor = smoke
		myveh.plateindex = GetVehicleNumberPlateTextIndex(veh)
		myveh.windowtint = wTint
		myveh.wheeltype = wType
		myveh.bulletProofTyres = bpTyre
		myveh.mods = modifs
		print("MyVeh updated!!@#!")
	end
end)
