...
AddEventHandler("LSC:finished", function(veh)
	...
	print(table.unpack(extracolor))
	print(table.unpack(neoncolor))
	print(table.unpack(smokecolor))
	print(plateindex)
	print(windowtint)
	print(wheeltype)
	print(bulletProofTyres)
	TriggerClientEvent("TRP_VEH:SaveVehicle", source, veh.vehicle, mods, color, extracolor, neoncolor, smokecolor, plateindex, windowtint, wheeltype, bulletProofTyres)
end)
